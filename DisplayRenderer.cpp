...
void DisplayRenderer::drawPage(const PageManager& pages) {
    ...
    // Content begins below the sub-tab strip on sub-paged pages.
    paramTop_ = PARAM_AREA_Y + contentTopOffset(pages.currentPage());
    cellH_    = (PARAM_AREA_H - contentTopOffset(pages.currentPage())) / 2;
    ...
    // Touch-edit focus border on top of the freshly drawn grid.
    prevEditCell_ = pages.editCell();
    if (prevEditCell_ != 0xFF) drawCellFocus(prevEditCell_, true, accent);
}
...
void DisplayRenderer::refreshValues(const PageManager& pages) {
    ...
    // Content offset for the active page (sub-paged pages sit below the strip).
    // Set before any page-type branch so ENV's refresh path picks it up too.
    paramTop_ = PARAM_AREA_Y + contentTopOffset(pages.currentPage());
    cellH_    = (PARAM_AREA_H - contentTopOffset(pages.currentPage())) / 2;

    // Header perf indicator: repaint if MODE/target changed, on ANY page. Done
    // before the ENV/SEQ early-returns so it stays live everywhere.
    refreshPerfIndicator(pages);
    ...
    for (uint8_t i = 0; i < 8; ++i) {
        ...
        if (val != prevValues_[i]) {
            ...
            drawParamCell(i % 4, i / 4, *slot, val, seeking, accent, isEnc,
                          /*labelStays=*/true);
            // A value repaint erases the focus border (it sits inside the
            // cell) — restore it if this is the focused cell.
            if (i == pages.editCell()) drawCellFocus(i, true, accent);
        }
    }

    // Focus moved since last frame: erase the old border, draw the new one.
    const uint8_t curEdit = pages.editCell();
    if (curEdit != prevEditCell_) {
        if (prevEditCell_ != 0xFF) drawCellFocus(prevEditCell_, false, accent);
        if (curEdit       != 0xFF) drawCellFocus(curEdit,       true,  accent);
        prevEditCell_ = curEdit;
    }
}
...
// Repaint the header perf indicator only if MODE or edit target changed since
// it was last drawn. Cheap: two CC-cache reads + a compare per frame, and a
// small fillRect+text repaint only on an actual change. drawPerfIndicator()
// updates the caches, so this naturally no-ops until something moves.
void DisplayRenderer::refreshPerfIndicator(const PageManager& pages) {
    const uint8_t mode = pages.getCCValue(CC::PERF_MODE);
    const uint8_t tgt  = pages.getCCValue(CC::PERF_EDIT_TARGET);
    if (mode != prevPerfMode_ || tgt != prevEditTarget_) {
        drawPerfIndicator(pages);  // also refreshes the caches
    }
}
...
// One page-tab pill, shared by the top and bottom rows. Draws into the row
// whose top edge is rowY and height rowH. Active tabs are filled in the page
// colour and show the FULL page name (folded identity, per design); inactive
// tabs are outlined and show the short tab label. tabIdx is the 0..3 slot
// within the row. Centralised so both rows look identical and the geometry
// matches hitTestTab() exactly.
void DisplayRenderer::drawPageTab(uint8_t tabIdx, const char* shortLabel,
                                  const char* fullName, uint16_t colour,
                                  bool active, uint16_t rowY, uint16_t rowH) {
    const uint16_t tx = TAB_PAD_X + tabIdx * TAB_W;
    const uint16_t pillY = rowY + TAB_PILL_M;
    const uint16_t pillH = rowH - 2 * TAB_PILL_M;
    const uint16_t pillW = TAB_W - 2 * TAB_PILL_M;

    if (active) {
        gfx_->fillRoundRect(tx + TAB_PILL_M, pillY, pillW, pillH, 4, colour);
        gfx_->setTextColor(COL_BG);            // dark text on filled pill
    } else {
        gfx_->drawRoundRect(tx + TAB_PILL_M, pillY, pillW, pillH, 4, colour);
        gfx_->setTextColor(colour);            // coloured text on outline
    }

    // Active tab shows the full name (size 1 so e.g. "ENVELOPE" fits the pill);
    // inactive shows the short tab at size 1 too, kept centred-ish by a simple
    // left inset. Size 1 is 6px/char — TAB_W=88 fits ~13 chars, ample.
    const char* txt = active ? fullName : shortLabel;
    gfx_->setTextSize(1);
    gfx_->setCursor(tx + TAB_PILL_M + 6, rowY + (rowH / 2) - 3);
    gfx_->print(txt);
}
...
void DisplayRenderer::drawHeader(const PageManager& pages) {
    ...
    // ── Top tab row: OSC / MIX / FLT / ENV (page indices 1..4) ──────────────
    // Tab slot i (0..3) maps to PageID(i + 1), matching the button arithmetic.
    // Labels come from the PARENT tab table, NOT getMapping(pid,0) — the latter
    // returns the first sub-page's label (e.g. "OSC1") on sub-paged pages.
    const PageID active = pages.currentPage();
    for (uint8_t i = 0; i < TABS_PER_ROW; ++i) {
        const PageID pid = static_cast<PageID>(i + 1);          // OSC..ENV
        const PageMap::PageTabInfo& t = PageMap::pageTabInfo(pid);
        drawPageTab(i, t.tab, t.name, toRgb565(t.ledColour),
                    pid == active, 0, HEADER_H);
    }

    // ── Performance-state indicator (right edge) ────────────────────────────
    // Replaces the old "S:A/S:B" scene-switch readout (which only echoed a
    // switch position and wasn't very useful). Shows the live performance MODE
    // and, when dual (Layer/Split), which layer is the edit target:
    //   SINGLE            (no target — only one layer exists)
    //   LAYER >A / >B / >AB
    //   SPLIT >A / >B / >AB
    // Both values come straight from the CC cache the synth already mirrors
    // (PERF_MODE / PERF_EDIT_TARGET are INTERNAL CCs 140/144). SELECT decode is
    // the bucket-index formula used everywhere: idx = value * count / 128.
    drawPerfIndicator(pages);

    // ── Sub-page tab strip (only on sub-paged pages) ────────────────────────
    drawSubTabs(pages);
}

// Draw the performance MODE + edit-target text at the header's right edge.
// Kept as its own helper so the (slightly fiddly) decode + layout lives in one
// place and drawHeader stays readable.
void DisplayRenderer::drawPerfIndicator(const PageManager& pages) {
    // Decode PERF_MODE (0=Single, 1=Layer, 2=Split) via the option table so
    // the text always matches ParamDefs, even if option order changes there.
    const ParamDef* pdMode = ParamDefs::findByCC(CC::PERF_MODE);
    const ParamDef* pdTgt  = ParamDefs::findByCC(CC::PERF_EDIT_TARGET);

    uint8_t modeIdx = 0;
    if (pdMode && pdMode->optionCount > 0) {
        modeIdx = (uint32_t)pages.getCCValue(CC::PERF_MODE) * pdMode->optionCount / 128;
        if (modeIdx >= pdMode->optionCount) modeIdx = pdMode->optionCount - 1;
    }
    const bool dual = (modeIdx != 0);  // Layer or Split → two layers active

    // Build the label. Short tab strings keep it inside the right margin.
    // MODE part uses the upper-cased short forms we want on the header.
    const char* modeTxt = (modeIdx == 2) ? "SPLIT"
                        : (modeIdx == 1) ? "LAYER"
                        :                  "SINGLE";
    ...
    // Right-align: the indicator occupies the rightmost ~78px of the header,
    // clear of the four 88px tabs (which end at 2 + 4*88 = 354; indicator
    // starts at 402, leaving a comfortable gap).
    const uint16_t ix = SCREEN_W - 78;
    // Clear the indicator area first so a shorter string never leaves stale
    // characters from a longer previous one (e.g. SPLIT >AB → SINGLE).
    gfx_->fillRect(ix, 0, 78, HEADER_H, COL_HEADER);

    gfx_->setCursor(ix, 6);
    gfx_->print(modeTxt);

    // Edit target only meaningful when two layers exist.
    if (dual && pdTgt && pdTgt->optionCount > 0) {
        uint8_t tgtIdx = (uint32_t)pages.getCCValue(CC::PERF_EDIT_TARGET)
                         * pdTgt->optionCount / 128;
        if (tgtIdx >= pdTgt->optionCount) tgtIdx = pdTgt->optionCount - 1;
        const char* tgtTxt = (tgtIdx == 2) ? "AB"
                           : (tgtIdx == 1) ? "B"
                           :                 "A";
        // Second line, accent-coloured, with a ">" cue for "editing".
        gfx_->setTextColor(COL_ACCENT);
        gfx_->setCursor(ix, 16);
        gfx_->print(">");
        gfx_->print(tgtTxt);
    }

    // Record what we drew so refreshValues() can detect a later change without
    // a full page redraw.
    prevPerfMode_   = pages.getCCValue(CC::PERF_MODE);
    prevEditTarget_ = pages.getCCValue(CC::PERF_EDIT_TARGET);
}
...
// Thin sub-page tab strip at the top of the content area. Shown only when the
// active page has sub-pages (OSC/FLT/ENV/LFO/FX). Mirrors the OLD footer
// sub-tab look, just relocated. Centred row of SUBTAB_W-wide tabs.
void DisplayRenderer::drawSubTabs(const PageManager& pages) {
    const PageID page = pages.currentPage();
    if (!PageMap::hasSubPages(page)) return;

    const uint8_t count = PageMap::subPageCount(page);
    const uint16_t startX = (SCREEN_W - count * SUBTAB_W) / 2;

    // Clear the strip first (content pages don't paint here themselves).
    gfx_->fillRect(0, SUBTAB_Y, SCREEN_W, SUBTAB_H, COL_BG);

    for (uint8_t i = 0; i < count; ++i) {
        const PageMapping& subMap = PageMap::getSubMapping(page, i);
        const bool active = (i == pages.currentSubPage());
        const uint16_t tx = startX + i * SUBTAB_W;
        const uint16_t col = toRgb565(subMap.ledColour);

        if (active) {
            gfx_->fillRoundRect(tx, SUBTAB_Y + 1, SUBTAB_W - 4, SUBTAB_H - 2,
                                3, col);
            gfx_->setTextColor(COL_BG);
        } else {
            gfx_->drawRoundRect(tx, SUBTAB_Y + 1, SUBTAB_W - 4, SUBTAB_H - 2,
                                3, col);
            gfx_->setTextColor(col);
        }
        gfx_->setTextSize(1);
        gfx_->setCursor(tx + 6, SUBTAB_Y + 5);
        gfx_->print(subMap.tab);
    }
}

// Touch-edit focus border. Uses the live paramTop_/cellH_ (set per frame) so
// it lands on the right cell whether or not the page has a sub-tab strip. A
// 2px inset border keeps it inside the cell's own 1px frame.
void DisplayRenderer::drawCellFocus(uint8_t cellIdx, bool on, uint16_t accent) {
    if (cellIdx >= 8) return;
    const uint8_t col = cellIdx % 4;
    const uint8_t row = cellIdx / 4;
    const uint16_t x = col * CELL_W;
    const uint16_t y = paramTop_ + row * cellH_;
    const uint16_t colr = on ? accent : COL_BG;
    // Two concentric rects = a 2px-thick border without a fill (so the cell's
    // value/label underneath are untouched).
    gfx_->drawRect(x + 1, y + 1, CELL_W - 2, cellH_ - 2, colr);
    gfx_->drawRect(x + 2, y + 2, CELL_W - 4, cellH_ - 4, colr);
}

// =============================================================================
// Footer — bottom page-tab row (LFO / FX / SEQ / PERF, page indices 5..8)
// =============================================================================
...
void DisplayRenderer::drawFooter(const PageManager& pages) {
    ...
    const PageID active = pages.currentPage();
    for (uint8_t i = 0; i < TABS_PER_ROW; ++i) {
        const PageID pid = static_cast<PageID>(5 + i);          // LFO..PERF
        const PageMap::PageTabInfo& t = PageMap::pageTabInfo(pid);
        drawPageTab(i, t.tab, t.name, toRgb565(t.ledColour),
                    pid == active, y, FOOTER_H);
    }
}

// =============================================================================
// Touch hit-test — see DisplayRenderer.h TabHit / hitTestTab docs
// =============================================================================

TabHit DisplayRenderer::hitTestTab(uint16_t x, uint16_t y,
                                   bool pageHasSub, uint8_t subCount) {
    // Top tab row: full HEADER_H height is the hit zone. Slots 0..3 → OSC..ENV.
    if (y < HEADER_H) {
        for (uint8_t i = 0; i < TABS_PER_ROW; ++i) {
            const uint16_t tx = TAB_PAD_X + i * TAB_W;
            if (x >= tx && x < tx + TAB_W) return { TabKind::PAGE, i };
        }
        return { TabKind::NONE, 0 };
    }

    // Bottom tab row: full FOOTER_H height. Slots 0..3 → LFO..PERF (index 4..7).
    if (y >= SCREEN_H - FOOTER_H) {
        for (uint8_t i = 0; i < TABS_PER_ROW; ++i) {
            const uint16_t tx = TAB_PAD_X + i * TAB_W;
            if (x >= tx && x < tx + TAB_W)
                return { TabKind::PAGE, static_cast<uint8_t>(TABS_PER_ROW + i) };
        }
        return { TabKind::NONE, 0 };
    }

    // Sub-tab strip (only when the active page actually shows it).
    if (pageHasSub && subCount > 0 &&
        y >= SUBTAB_Y && y < SUBTAB_Y + SUBTAB_H) {
        const uint16_t startX = (SCREEN_W - subCount * SUBTAB_W) / 2;
        for (uint8_t i = 0; i < subCount; ++i) {
            const uint16_t tx = startX + i * SUBTAB_W;
            if (x >= tx && x < tx + SUBTAB_W) return { TabKind::SUBTAB, i };
        }
    }
...
uint8_t DisplayRenderer::hitTestCell(uint16_t x, uint16_t y, bool pageHasSub) {
    // Content area mirrors the param-grid renderer: top sits below the sub-tab
    // strip on sub-paged pages, and the two rows split the remaining height.
    const uint16_t offset = pageHasSub ? SUBTAB_H : 0;
    const uint16_t top    = PARAM_AREA_Y + offset;
    const uint16_t cellH  = (PARAM_AREA_H - offset) / 2;

    if (y < top || y >= top + 2 * cellH) return kNoCell;  // header/strip/footer
    if (x >= SCREEN_W) return kNoCell;

    const uint8_t col = x / CELL_W;            // 0..3
    const uint8_t row = (y - top) / cellH;     // 0..1
    if (col > 3 || row > 1) return kNoCell;
    return static_cast<uint8_t>(row * 4 + col);
}
...
void DisplayRenderer::drawParamCell(uint8_t col, uint8_t row,
    ...
    if (labelStays) {
        gfx_->fillRect(x + 1, y + 14, CELL_W - 2, cellH_ - 15, bgCol);
    } else {
        gfx_->fillRect(x + 1, y + 1, CELL_W - 2, cellH_ - 2, bgCol);
    }
    ...
    if (slot.type == CtrlType::TOGGLE) {
        ...
        gfx_->setTextSize(3);
        gfx_->setCursor(x + 4, y + cellH_ / 2 - 8);
        ...
        if (pd && pd->optionCount > 0 && pd->options) {
            ...
            gfx_->setTextSize(2);
            gfx_->setCursor(x + 4, y + cellH_ / 2 - 6);
...
void DisplayRenderer::refreshEnvelopePage(const PageManager& pages) {
    ...
    if (now.a != prevEnv_.a || now.d != prevEnv_.d || now.r != prevEnv_.r) {
        gfx_->fillRect(0, paramTop_, SCREEN_W, PARAM_AREA_H - (paramTop_ - PARAM_AREA_Y), COL_BG);
    ...
    // BUGFIX (ghost sustain dot): a sustain (vertical-only) change moves the
    // D→R vertex dot's Y without moving any segment's X. The per-segment
    // COL_BG erase below only retraces segment *lines*, not the dots, so the
    // OLD sustain dot used to linger as a ghost until an a/d/r change forced a
    // full redraw. Erase the OLD dots up front (before the new segments are
    // drawn) so the new D/R lines, painted afterwards, sit on cleared pixels
    // and the erase circle can't clip them. Only needed when sustain moved;
    // pure curve-shape changes don't move any dot.
    if (sChanged) drawEnvelopeDots(prevEnv_, COL_BG);
    ...
    // Repaint the four vertex dots in accent — restores any dot pixels the
    // segment erase pass clipped, and places the sustain dot at its new Y.
    ...
    if (depthDirty) {
        ...
        gfx_->fillRect(drawR - 110, paramTop_ + 4, 110, 16, COL_BG);
        gfx_->setTextColor(dimColour(accent, 1));
        gfx_->setTextSize(2);
        gfx_->setCursor(drawR - 110, paramTop_ + 4);
        gfx_->print("DEPTH ");
...
void DisplayRenderer::drawEnvelopeSegment(uint8_t segIdx, const EnvParams& env,
    ...
    const uint16_t topY   = paramTop_ + ENV_PAD_TOP;
...
void DisplayRenderer::drawPtchBanner(const PatchManager& patches) {
    ...
    switch (patches.bannerType()) {
        ...
        case PtchBannerType::RENAMED:
            snprintf(text, sizeof(text), "RENAMED");
            break;
