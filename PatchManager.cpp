...
void PatchManager::begin(PatchStore& store, ViewController& vc) {
...
void PatchManager::handleLoadPush() {
    ...
    // v2 patches are kParamCount normalised floats, keyed by table ordinal.
    // static: keep 560 bytes off the stack, and this is control-plane only.
    static float buf[JT::Params::kParamCount];

    if (!store_->load(highlightedSlot_, buf, JT::Params::kParamCount)) {
        // load() already logged WHY (missing / bad magic / wrong param count).
        // A v1 patch lands here: it has no magic and is refused rather than
        // being misread as a short v2 file.
        showBanner(PtchBannerType::LOAD_FAILED, highlightedSlot_, Config::PTCH_BANNER_MS);
        return;
    }

    // loadAll(markDirty=true) marks every parameter dirty, so the very next
    // ViewController::update() drains the set and sends the WHOLE patch as
    // NRPN. The old manual CC relay loop is gone — there is one send path now,
    // and it is the same one a knob uses.
    vc_->store().loadAll(buf, JT::Params::kParamCount, /*markDirty=*/true);
