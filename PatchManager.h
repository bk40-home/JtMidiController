...
// the actual storage (PatchStore on FFat) + relay target (ViewController's

// parameter store). Neither PatchStore nor ViewController
...
// WHY THIS EXISTS (rather than putting save/load logic inside ViewController):

//   ViewController's job is hardware -> parameter routing. Patch
...
//   bloat ViewController with concerns unrelated to its existing job.
...
public:
    ...
    void begin(PatchStore& store, ViewController& vc);
...
private:
    ...
    // The action callback is a plain function pointer, so it can't
